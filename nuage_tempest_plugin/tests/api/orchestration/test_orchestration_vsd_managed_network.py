...
class OrchestrationVsdManagedNetworkTest(
    ...
    def test_link_subnet_to_vsd_l2domain_without_net_partition(self):
        ...
        Test heat creation should succeed for a private VSD managed
        network without net-partition if nuagenet is in the default
        net partition
        """
        # Create the VSD l2 domain from a template
        name = data_utils.rand_name('l2domain-')
        ...
        self.assertIsNone(subnet['gateway_ip'],
                          "Shall not have gateway_ip as no dhcp option "
                          "configured")
        # in this case we get .2 correctly
