...
try:
    # TODO(team) - is this needed ?
    from .node import Node  # noqa H304
except ImportError:
...
class Tree(object):
        ...
        if filter(self[nid]) and self[nid].expanded:
            ...
            for element in queue:
                ...
                self.__print_backend(element.identifier, level, idhidden,
                                     filter, key, reverse, line_type, func,
                                     iflast)
                iflast.pop()
    ...
    def update_node(self, nid, os_data=None, vsd_data=None,
        ...
        if node:
            if os_data:
                node.os_data = os_data
            if vsd_data:
                node.vsd_data = vsd_data
            if vsc_data:
    ...
    def paste(self, nid, new_tree, deepcopy=False):
        ...
        if set_joint:
            # TODO(team): a deprecated routine is needed to avoid exception
    ...
    def remove_node(self, identifier):
        ...
        for id in self.expand_tree(identifier):
            # TODO(team): implementing this function as a recursive function:
    ...
    def save2file(self, filename, nid=None, level=ROOT, idhidden=True,
        ...
        handler = lambda x: _write_line(x, open(filename, 'ab'))  # noqa E731
    ...
    def to_json(self, with_data=False, sort=True, reverse=False):
        ...
        return json.dumps(self.to_dict(with_data=with_data, sort=sort,
                                       reverse=reverse))
