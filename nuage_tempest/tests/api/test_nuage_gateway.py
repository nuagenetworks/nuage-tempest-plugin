...
from nuage_tempest.tests.api.upgrade.external_id.external_id \
    import ExternalId
...
class NuageGatewayTestJSON(base.BaseNuageGatewayTest):
    ...
    def test_default_security_group_bridge_port(self):
        ...
        gw_vport = self.nuage_vsd_client.get_host_vport(vport1['id'])
        self.verify_vport_properties(gw_vport[0], vport1)
        body = self.admin_client.list_gateway_vport(self.subnet['id'])
        vports = body['nuage_gateway_vports']
        found_vport = False
        vport = None
