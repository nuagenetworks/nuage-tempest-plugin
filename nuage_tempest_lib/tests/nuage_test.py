...
class NuageBaseTestCase(test.BaseTestCase, NuageBaseMixin):
    pass


class NuageBaseNetworkTest(base.BaseNetworkTest, NuageBaseMixin):
    pass


class NuageBaseAdminNetworkTest(base.BaseAdminNetworkTest, NuageBaseMixin):
    pass


class NuageNetworkScenarioTest(manager.NetworkScenarioTest, NuageBaseMixin):
    pass


class NuageBaseTest(NuageNetworkScenarioTest):
    ...
    def setup_network_resources(cls):
        ...
        LOG.info("setup_network_resources: ipv4 config: {}".format(
            str(cls.cidr4)))
        LOG.info("setup_network_resources: ipv6 config: {}".format(
            str(cls.cidr6)))
        ...
        for subnet_cidr in cidr.subnet(mask_bits):
            ...
            try:
                if not no_net_partition and 'net_partition' not in kwargs:
                    kwargs['net_partition'] = self.def_netpartition
    ...
    def create_keypair(self, _=None):
    ...
    def assert_ping(self, server1, server2, network, ip_type=4,
        ...
        for attempt in range(1, self.nbr_retries_for_test_robustness + 1):
            ...
            if ping_result == should_pass:
                LOG.info('assert_ping: ping attempt %d %s', attempt,
                         ping_pass)
                return True
...
class NuageAdminNetworkTest(NuageBaseAdminNetworkTest):
    ...
    def clear_credentials(cls):
        if cls.is_dhcp_agent_present():
            ...
            for attempt in range(cls.nbr_retries_for_test_robustness):
                try:
                    super(NuageAdminNetworkTest, cls).clear_credentials()
                    return
