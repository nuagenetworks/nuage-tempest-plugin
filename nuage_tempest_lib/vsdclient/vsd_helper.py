...
class VsdHelper(object):
    ...
    def get_l2domain(self, enterprise=None,
        ...
        if vspk_filter:
            ...
            return None
        if not l2_domain:
            LOG.warn('could not fetch the l2 domain '
    ...
    def get_l3domain(self, enterprise=None,
        ...
        if vspk_filter:
            ...
            return None
        if not domain:
            LOG.warn('could not fetch the domain matching the filter "{}"'
                     .format(vspk_filter))
        return domain
    ...
    def get_domain_template(self, enterprise=None, vspk_filter=None,
        ...
        if not domain_template:
            LOG.warn('could not fetch the domain template '
                     'matching the filter "{}"'
                     .format(vspk_filter))
    ...
    def get_zone(self, domain=None, vspk_filter=None, by_router_id=None):
        ...
        if vspk_filter:
            ...
            return None
        if not zone:
            LOG.warn('could not fetch the zone matching the filter "{}"'
                     .format(vspk_filter))
        return zone
    ...
    def get_subnet(self, zone=None, vspk_filter=None, by_subnet_id=None):
        ...
        if vspk_filter:
            ...
            return None
        if not subnet:
            LOG.warn('could not fetch the subnet matching the filter "{}"'
                     .format(filter))
        return subnet
    ...
    def get_vm(self, subnet=None, vspk_filter=None, by_device_id=None):
        ...
        if vspk_filter:
            ...
            return None
        if not vm:
            LOG.warn('could not fetch the vm matching the filter "{}"'
                     .format(vspk_filter))
        return vm
    ...
    def get_subnet_dhcp_options(self, subnet=None, vspk_filter=None):
        ...
        if not dhcp_options:
            if vspk_filter:
                LOG.warn('could not fetch the dhcp options '
    ...
    def get_vport(self, l2domain=None, subnet=None, vspk_filter=None,
        ...
        if vspk_filter:
            ...
            return None
        if not vport:
            LOG.warn('could not fetch the vport from the l2domain/subnet '
    ...
    def get_vm_interface(self, vspk_filter):
        ...
        vm_interface = self.session().user.vm_interfaces.get_first(
            filter=vspk_filter)
        if not vm_interface:
            LOG.warn('could not fetch the vm interface '
                     'matching the filter "{}"'.format(vspk_filter))
        return vm_interface
    ...
    def get_vm_interface_policy_decisions(self, vm_interface=None,
        ...
        if not policy_decisions:
            if vspk_filter:
                LOG.warn('could not fetch the policy decisions '
    ...
    def get_ingress_acl_entry(self, vspk_filter):
        ...
        acl = self.session().user.ingress_acl_entry_templates.get_first(
            filter=vspk_filter)
        if not acl:
            LOG.warn('could not fetch the ingress acl entry matching '
    ...
    def get_egress_acl_entry(self, vspk_filter):
        ...
        acl = self.session().user.egress_acl_entry_templates.get_first(
            filter=vspk_filter)
        if not acl:
            LOG.warn('could not fetch the egress acl entry matching '
                     'the filter "{}"'.format(vspk_filter))
        return acl
    ...
    def get_floating_ip(self, vspk_filter):
        ...
        floating_ip = self.session().user.floating_ips.get_first(
            filter=vspk_filter)
        if not floating_ip:
            LOG.warn('could not fetch the floating ip matching '
    ...
    def get_ingress_acl_entries(self, vspk_filter):
        ...
        templates = self.session().user.ingress_acl_templates.get(
            filter=vspk_filter)
        if not templates:
            LOG.warn('could not fetch the ingress acl entries (templates) '
                     'matching the filter "{}"'.format(vspk_filter))
            return None
        acls = []
    ...
    def get_egress_acl_entries(self, vspk_filter):
        ...
        templates = self.session().user.egress_acl_templates.get(
            filter=vspk_filter)
        if not templates:
            LOG.warn('could not fetch the egress acl entries (templates) '
                     'matching the filter "{}"'.format(vspk_filter))
            return None
        acls = []
        for template in templates:
    ...
    def get_shared_network_resource(self,
        ...
        if not shared_network_resource:
            LOG.warn('could not fetch the shared network resource '
                     'matching the filter "{}"'.format(vspk_filter))
    ...
    def get_virtual_ip(self, vport, vspk_filter):
        ...
        if not virtual_ip:
            LOG.warn('could not fetch the virtualip matching the '
    ...
    def get_firewall_acl(self, ent=None, vspk_filter=None,
        ...
        if not firewall_acl:
            LOG.warn('could not fetch the firewall_acl matching '
                     'the filter "{}"'.format(vspk_filter))
        return firewall_acl
    ...
    def get_firewall_acls(self, ent=None, vspk_filter=None,
        ...
        if not firewall_acls:
            LOG.warn('could not fetch the firewall_acls matching '
                     'the filter "{}"'.format(vspk_filter))
    ...
    def get_firewall_rule(self, ent=None, vspk_filter=None,
        ...
        if not firewall_rule:
            LOG.warn('could not fetch the firewall_rule matching '
                     'the filter "{}"'.format(vspk_filter))
