...
from nuage_vsdclient_lib.restproxy import RESTProxyServer
...
class NuageRestClient(NuageBaseMixin):

    def __init__(self):
        server = self.nuage_config.nuage_vsd_server
        vsd_org = self.nuage_config.nuage_vsd_org
        base_uri = self.nuage_config.nuage_base_uri
        auth_resource = self.nuage_config.nuage_auth_resource
        server_auth = (self.nuage_config.nuage_vsd_user + ":" +
                       self.nuage_config.nuage_vsd_password)
        ...
        self.restproxy = RESTProxyServer(server, base_uri, SERVERSSL,
    ...
    @staticmethod
    def _log_request_full(method, req_url, resp,
        ...
        for attempt in range(self.nbr_retries_for_test_robustness):
            try:
                ...
                if 'This l2domaintemplate is in use' not in str(e):
                    ...
                    if attempt >= self.nbr_retries_for_test_robustness - 1:
                        raise
